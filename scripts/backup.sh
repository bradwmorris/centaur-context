...
resolve_legacy_env CENTAUR_CONTEXT_DATABASE_URL CENTAUR_OS_DATABASE_URL
resolve_legacy_env CENTAUR_CONTEXT_DATABASE_PASSWORD CENTAUR_OS_DATABASE_PASSWORD
...
printf '{"product":"centaur-context","product_version":"%s","database":"%s","schema_version":%s,"format":"pg_dump-custom"}\n' \
  "$CENTAUR_CONTEXT_VERSION" "$database" "$schema" >"$metadata_partial"
mv "$partial" "$output"
