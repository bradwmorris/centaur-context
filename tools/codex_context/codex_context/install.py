    ...
    with (settings.root/'install.lock').open('a') as lock:
        ...
        if previous:
            for event,group in previous['hook_groups'].items():
                ...
                else:raise ValueError('Owned hook was changed or removed; reconcile before changing the installation')
        ...
        if launchd or (previous and previous.get('launchd_path')):
            ...
            if plist.exists():
                ...
                if not previous or existing.get('Label')!=LABEL or str(plist)!=previous.get('launchd_path'):
        ...
        launchd = launchd or bool(previous and previous.get('launchd_path'))
        if not remove:
            # Persist ownership before starting the service. If bootstrap fails,
            # reinstall/uninstall can still recover the already-written entries.
        ...
        if launchd:
            ...
            if plist.exists():
                ...
                if subprocess.run(['launchctl','print',domain+'/'+LABEL],capture_output=True).returncode==0:
                    raise ValueError('Launch agent is still running; installation state retained for recovery')
            if remove:
        ...
        if remove and manifest_path.exists():manifest_path.unlink()
