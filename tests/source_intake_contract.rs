...
fn request(path: &str, token: &str, body: &Value) -> Request<Body> {
    Request::builder()
        ...
        .header("x-centaur-principal-id", "workflow-source-ingestion")
...
async fn validates_commits_replays_conflicts_and_reports_retrieval_readiness() {
    ...
    let key = format!("source-contract-{}", uuid::Uuid::new_v4());
    let content =
        format!("The Source workflow stores this distinctive retrievable evidence: {key}.");
    let payload = json!({
        "version":"centaur-context-source-intake-v3",
        "idempotency_key":key,
        "source":{
            "title":"A durable research source",
            "description":"A verified Source used to exercise the permanent ingestion contract.",
            "source_kind":"article",
            "canonical_uri":format!("https://example.test/source/{key}"),
            ...
            "extraction_method":"research-agent",
            "extraction_version":"1",
            "capture_outcome":"complete",
            "capture_reason":null,
            "expected_size_bytes":content.len(),
            "captured_at":"2026-08-30T00:00:00Z",
            "provenance":{"source_type":"workflow","source_ref":"contract-test"}
        },
        "connections":[],
    ...
    let app = router(state, token.clone(), "workflow-source-ingestion".into());
    ...
    .await
    .unwrap();
    assert_eq!(stored.0, "A durable research source");
...
async fn reuses_exact_source_for_new_connection_batches_without_duplicates() {
    ...
    db::migrate(&pool).await.unwrap();
    let token = "s".repeat(32);
    let key = format!("source-reuse-{}", uuid::Uuid::new_v4());
    let canonical_uri = format!("https://example.test/source/{key}");
    ...
    let payload = json!({
        ...
        "source":{
            ...
            "provenance":{"source_type":"workflow","source_ref":"reuse-test"}
        },
        "connections":[],
    ...
    let app = router(
        ...
        token.clone(),
        "workflow-source-ingestion".into(),
    );
    ...
    let stored_counts: (i64, i64, i64, i64) = sqlx::query_as(
        r#"SELECT
             ...
             (SELECT count(*) FROM connections
              WHERE source_object_id=$2::uuid AND kind='about'
    ...
    content_only["source"]["canonical_uri"] =
        json!(format!("https://example.test/source/{key}-different"));
...
async fn adopts_only_an_incomplete_unprotected_curator_source() {
    ...
    db::migrate(&pool).await.unwrap();
    let token = "s".repeat(32);
    let key = format!("source-adoption-{}", uuid::Uuid::new_v4());
    let canonical_uri = format!("https://example.test/source/{key}");
    ...
    let payload = json!({
        ...
        "idempotency_key":key,
        "source":{
            "title":"A captured research source",
            "description":"A complete source captured by the Source-ingestion workflow.",
            "source_kind":"article",
            "canonical_uri":canonical_uri,
            "byline":"Example Author",
            "publisher":"Example Publisher",
            ...
            "expected_size_bytes":content.len(),
            "captured_at":"2026-08-30T00:00:00Z",
            "provenance":{"source_type":"workflow","source_ref":"adoption-test"}
        },
        "connections":[],
        "originating_chat_object_id":null
    });
    ...
    .await
    .unwrap();
    assert_eq!(stored.0, "A captured research source");
    assert!(stored.1);
    assert_eq!(stored.2, 1);
    assert_eq!(stored.3, "workflow");
