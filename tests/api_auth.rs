...
async fn research_mutation_listener_requires_its_exact_workflow_principal() {
    let router =
        research_mutation_router(state(), "m".repeat(32), "workflow-context-mutation".into());
    for (token, principal, expected) in [
        ...
        ("m".repeat(32), "agent-researcher", StatusCode::FORBIDDEN),
        (
            "m".repeat(32),
            "workflow-source-ingestion",
            StatusCode::FORBIDDEN,
        ),
        ("m".repeat(32), "workflow-context-mutation", StatusCode::OK),
    ] {
        let response = router
            .clone()
            .oneshot(
...
async fn source_intake_listener_requires_its_token_and_exact_workflow_principal() {
    let router = source_intake_router(state(), "s".repeat(32), "workflow-source-ingestion".into());
    ...
    let wrong_principal = router
        ...
        .oneshot(
            Request::builder()
                ...
                .header("x-centaur-principal-id", "agent-editor")
                .header("x-centaur-thread-key", "slack:test")
                .body(Body::empty())
                .unwrap(),
        )
...
async fn human_api_declares_v2_and_unknown_versions_fail_closed() {
    ...
    assert_eq!(metadata["database_schema_version"], 30);
...
async fn note_and_task_write_listener_is_a_separate_attributed_idempotent_grant() {
    ...
    let untyped = note_write_router(state(), write_token.clone())
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v2/notes")
                ...
                .header("x-centaur-principal-id", "researcher")
                .header("x-centaur-thread-key", "slack:T:C:thread")
                .header("idempotency-key", "untyped-note")
                .header("content-type", "application/json")
                .body(Body::from(r##"{"title":"Untyped","description":"A Note missing its required research intent.","content":"Evidence"}"##))
                .unwrap(),
        )
        .await
        .unwrap();
    assert_eq!(untyped.status(), StatusCode::UNPROCESSABLE_ENTITY);
