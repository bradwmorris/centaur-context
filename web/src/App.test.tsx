...
const users = [
  { user_object_id: "alex-1", title: "Alex", user_kind: "human", role: "participant", avatar_url: null, avatar_asset_url: null },
  { user_object_id: "agent-1", title: "Atlas", user_kind: "agent", role: "participant", avatar_url: null, avatar_asset_url: null },
];
const run = { id: "run-1", parent_run_id: null, kind: "intake", status: "completed", actor_type: "centaur_agent", actor_id: "workflow-source-ingestion", chat_object_id: chat.id, primary_object_id: source.id, idempotency_key: "run-1", input: {}, trace: [], result: { counts: { objects: 0, connections: 6 }, object_ids: [source.id] }, consulted_object_ids: [], error: null, verdict: "unreviewed", review_notes: null, pinned: false, reviewed_by: null, reviewed_at: null, available_at: null, started_at: now, completed_at: now, created_at: now, updated_at: now };
...
describe("minimal canonical UI", () => {
  ...
  it("shows all root Runs in an editable pinned-first eval table", async () => {
    ...
    expect(within(rows[1]).getByRole("img", { name: "Alex, Human, participant" })).toBeVisible();
    expect(within(rows[1]).getByRole("img", { name: "Atlas, Agent, participant" })).toBeVisible();
  ...
  it("lists consolidated runs through API v2", async () => {
    ...
    expect(screen.getByRole("img", { name: "Alex, Human, participant" })).toBeInTheDocument();
    expect(screen.getByRole("img", { name: "Atlas, Agent, participant" })).toBeInTheDocument();
  ...
  it("explains a captured eval run and labels historical gaps", async () => {
    ...
    const explainable = {
      ...
      input: { request_message: { content: "What changed?", provider_message_id: "123.1", sender: { display_name: "Alex" }, source_created_at: now } },
