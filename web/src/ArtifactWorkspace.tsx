...
function displayType(artifact: Artifact): string {
  return artifact.kind === "research_notes" ? "Working notes" : artifact.kind.replaceAll("_", " ");
}
...
export function Artifacts({ section, objectId, artifacts, onCreated }: { section: ParentSection; objectId: string; artifacts: Artifact[]; onCreated: () => Promise<void> }) {
  ...
  return <section className="artifact-section" aria-label="Artifacts">
    <div className="artifact-section-head"><h2>Artifacts</h2><div className="artifact-actions"><button className="secondary" type="button" onClick={() => openForm("notes")}>+ Add working notes</button><button className="ghost" type="button" onClick={() => openForm("generic")}>Add artifact</button></div></div>
    {form && <form className="artifact-create-form" onSubmit={(event) => void submit(event)}>
      <h3>{form === "notes" ? "New working notes" : "New artifact"}</h3>
      {form === "generic" && <div className="artifact-create-fields"><label>Kind<input name="kind" required maxLength={100} defaultValue="transcript" /></label><label>Document key for working notes<input name="document_key" maxLength={100} placeholder="default" /></label><label>Captured<input name="captured_at" type="datetime-local" /></label><label>Language<input name="language" maxLength={35} placeholder="en" /></label><label>Capture outcome<select name="capture_outcome" defaultValue="complete"><option value="complete">Complete</option><option value="incomplete">Incomplete</option><option value="unavailable">Unavailable</option><option value="paywalled">Paywalled</option><option value="disallowed">Disallowed</option><option value="too_large">Too large</option><option value="unsupported">Unsupported</option></select></label><label>Reason when not complete<input name="capture_reason" maxLength={1000} /></label><label>Supersedes artifact<select name="predecessor" defaultValue=""><option value="">None / latest working notes with this key</option>{artifacts.map((item) => <option key={item.id} value={item.id}>{artifactLabel(item)} · {item.id.slice(0, 8)}</option>)}</select></label></div>}
      <label>Title<input name="title" maxLength={300} placeholder={form === "notes" ? "Working notes" : "Artifact title"} /></label>
