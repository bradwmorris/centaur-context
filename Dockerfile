...
RUN apt-get update \
    ...
    && groupadd --gid 10001 centaur-context \
    && useradd --uid 10001 --gid 10001 --no-create-home centaur-context
COPY --from=rust-build /build/target/release/centaur-context /usr/local/bin/centaur-context
COPY --from=web-build /build/web/dist /app/web
LABEL org.opencontainers.image.title="Centaur Context" \
      org.opencontainers.image.description="Local-first shared context layer for Centaur" \
      org.opencontainers.image.version="0.3.0" \
      org.opencontainers.image.source="https://github.com/bradwmorris/centaur-context"
USER 10001:10001
...
ENTRYPOINT ["/usr/local/bin/centaur-context"]
