...
purpose-bound credential, hosted writes/deployment, and live backfill. Recommended
...
2. Extend the existing `embeddings` table for Object and Artifact-chunk targets,
   then implement deterministic chunking, hybrid evidence retrieval, backfill,
   and operational visibility.
3. Verify locally and on a disposable database; after separate approvals, deploy,
   canary, backfill live data, measure retrieval, and retain a tested rollback.
...
- **Done:** Intake completeness is enforced and visible; current Artifact hashes,
  sizes, provenance, versions, and outcomes reconcile; chunk/Object embedding jobs
  are idempotent and current; hybrid retrieval returns attributed Artifact spans;
  agent/client/UI/operations paths expose state; eval gates pass; and approved live
  canaries and backfill reconcile with lexical fallback intact.
...
- Retain concise Object embeddings for identity lookup. Embed only current,
  complete, textual Source Artifacts as paragraph-aligned chunks, initially about
  6,000 Unicode characters with 600-character overlap and a provider-token-limit
  guard. Record formatter/chunker version and offsets. Measurements may tune these
  constants without weakening exact-span attribution.
- Claim rows with `SKIP LOCKED`, recover expired leases, use bounded exponential
  ...
  hash/config. New current Artifacts queue chunks transactionally; superseded rows
  remain derived history but never rank. Backfill is resumable, dry-runnable,
  count/hash reconciled, and safe to repeat.
...
- Rehearse migration/backfill on a disposable schema-17 snapshot. For live rollout:
  approve provider and credential; back up/checksum; deploy Enyu/Context together;
  canary one query and one complete Source without bulk backfill; inspect usage,
  retrieval, and failures; then run resumable backfill. Roll back by disabling
  embedding config and reverting callers/service; lexical retrieval remains live.
...
API, creating or changing credentials, hosted writes/backfill, deployment, live
