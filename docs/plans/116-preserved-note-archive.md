...
Ordinary archive_object may archive a protected Note only when its exact current content is contained in a complete research_notes Artifact attached to an active Source to which the Note has an active derived_from Connection, or that exact Connection was archived earlier in the same apply batch. The Artifact must have a nonempty document_key and a source_note_manifest entry matching object_id and current revision. Ordinary archive_connection may archive a protected Connection incident to such a verified protected Note; its existing immutable archive event is retained. Require all active Connections to be archived first in the same batch. Reject empty content, stale revisions, absent/mismatched manifest, wrong Source, incomplete capture, prior-run archived-link witnesses, and all other protected record archival. Preserve source canonical evidence and artifact history.
...
## Acceptance
Focused real disposable database tests for permitted preserved Note and Connection archival and denial boundaries, normal archival regression coverage, formatting, clippy, required CI, self-review and draft PR. Keep change narrowly scoped. No production test fixtures.
