...
async fn source_intake_auth(
    ...
    if principal != *state.allowed_principal {
        return Err(IntakeError::Forbidden(
            "only the configured Source-intake principal may use this listener".into(),
        ));
    }
...
async fn prepared(
    ...
    if let Some(conflict) = foreign.next() {
        ...
        if conflict.same_batch
            && conflict
                .provenance
                .get("adopted_curator_provenance")
                .is_some()
        {
            prepared.adopt_object("source", conflict.object_id)?;
        } else if exact_existing_source && prepared.request.connections.is_empty() {
            return Err(IntakeError::Conflict(
                "canonical URI and exact content already belong to an existing Source".into(),
            ));
        } else if exact_existing_source {
    ...
    fn request() -> SourceIntakeRequest {
        SourceIntakeRequest {
            ...
            source: SourceManifest {
                ...
                extraction_method: Some("research-agent".into()),
                ...
                captured_at: None,
                provenance: Some(json!({"source_type":"workflow"})),
            },
            connections: Vec::new(),
