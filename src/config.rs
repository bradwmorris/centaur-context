...
fn source_intake_config() -> Result<Option<SourceIntakeConfig>> {
    let token = optional("SOURCE_INTAKE_API_TOKEN");
    let principal = optional("SOURCE_INTAKE_ALLOWED_PRINCIPAL");
    if token.is_none() && principal.is_none() {
        return Ok(None);
    }
    let api_token = token.context(
        "SOURCE_INTAKE_API_TOKEN is required when Source-intake configuration is provided",
    )?;
    if api_token.len() < 32 {
    ...
    let allowed_principal = configured_principal(
        principal,
        "SOURCE_INTAKE_ALLOWED_PRINCIPAL",
        "Source-intake",
    )?;
...
fn research_mutation_config() -> Result<Option<ResearchMutationConfig>> {
    let token = optional("RESEARCH_MUTATION_API_TOKEN");
    let principal = optional("RESEARCH_MUTATION_ALLOWED_PRINCIPAL");
    if token.is_none() && principal.is_none() {
        return Ok(None);
    }
    let api_token = token.context(
        "RESEARCH_MUTATION_API_TOKEN is required when Research-mutation configuration is provided",
    )?;
    if api_token.len() < 32 {
    ...
    let allowed_principal = configured_principal(
        principal,
        "RESEARCH_MUTATION_ALLOWED_PRINCIPAL",
        "Research-mutation",
    )?;
...
fn configured_principal(
    value: Option<String>,
    variable: &'static str,
    listener: &'static str,
) -> Result<String> {
    let value = value.with_context(|| {
        format!("{variable} is required when the {listener} listener is enabled")
    })?;
    if value.len() > 128 || !value.is_ascii() || value.chars().any(char::is_whitespace) {
        bail!("{variable} must be an ASCII principal ID of at most 128 characters");
    }
    Ok(value)
}
